...
public class LoadSystem : MonoBehaviour
{
    ...
    public static InventoryObject playerInventory;
    private void Start()
    {
        Reload();
    }
    ...
    public static void Reload()
    {
        ...
        player.GetComponent<PlayerController2>().inventory.startInventory();
        ...
        foreach (GameObject item in items)
        {
            for (int i = 0; i < reloadedData.itemPosition.Count; i++)
            {
                ...
                if (item.transform.position.Equals(itemPos))
                {
                    ...
                    player.GetComponent<PlayerController2>().inventory.addItem(item);
                }
            }
        }
    }
