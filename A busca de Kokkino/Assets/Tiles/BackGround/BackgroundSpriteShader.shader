	...
	SubShader
	{
		...
		CGPROGRAM
			...
			struct Interpolator
			{
				...
				float worldPos : TEXCOORD1; // coordenada central do modelo
			};
			...
			Interpolator vert(Input IN)
			{
				...
				OUT.worldPos = mul(UNITY_MATRIX_M,float4(0,0,0,1)).x; // calula a coordenada central do modelo
			...
			{
				float xOffset = IN.worldPos / _Scale; // calcula o quanto é o offset da textura que vai pegar
				fixed4 c = SampleSpriteTexture (IN.texcoord+float2(xOffset,0)) * IN.color; // pega a textura com o offset e mistura com a cor
